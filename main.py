...
from libs.utils import PolynomialLR, scores  #DenseCRF
...
@click.option(
    ...
    default='data/models/floodnet/deeplabv2_resnet101_msc/train/checkpoint_15000.pth',
...
def test(config_path, model_path, cuda):
    ...
    for image_ids, images, gt_labels in tqdm(
        ...
        imgs += list(images.cpu().numpy())
        imgIds += list(image_ids)
    ...
    show_results(imgs[:20],  imgIds[:20], gts[:20], preds[:20],  img_save_dir)
